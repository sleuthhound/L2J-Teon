...
public abstract class Inventory extends ItemContainer
{
     ...
     * void notifyUnequiped(int slot, L2ItemInstance item) { if
     * (!(getOwner() != null && getOwner() instanceof L2PcInstance)) return;
     * L2PcInstance owner = (L2PcInstance)getOwner(); if (item.getItemId() ==
     * 6408) owner.setIsWearingFormalWear(false); } public void
     * notifyEquiped(int slot, L2ItemInstance item) { if (!(getOwner() !=
     * null && getOwner() instanceof L2PcInstance)) return; L2PcInstance
     * owner = (L2PcInstance)getOwner(); // If player equip Formal Wear
     * unequip weapons and abort cast/attack if (item.getItemId() == 6408) {
     * owner.setIsWearingFormalWear(true); if (owner.isCastingNow())
     * owner.abortCast(); if (owner.isAttackingNow()) owner.abortAttack();
     * setPaperdollItem(PAPERDOLL_LHAND, null);
     * setPaperdollItem(PAPERDOLL_RHAND, null);
     * setPaperdollItem(PAPERDOLL_LRHAND, null); } else { if
     * (!owner.isWearingFormalWear()) return; // Don't let weapons be
     * equipped if player is wearing Formal Wear if (slot == PAPERDOLL_LHAND ||
     * slot == PAPERDOLL_RHAND || slot == PAPERDOLL_LRHAND) {
     ...
     * @return L2ItemInstance corresponding to the destroyed item or the
     *         updated item in inventory
     */
    ...
    /**
     * Adds item to inventory for further adjustments and Equip it if
     * necessary (itemlocation defined)<BR>
     * <BR>
     * 
     * @param item :
