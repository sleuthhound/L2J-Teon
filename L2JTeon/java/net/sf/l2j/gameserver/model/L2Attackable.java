...
import net.sf.l2j.gameserver.skills.conditions.ConditionInventory;
 ...
 * <li>L2FortSiegeGuardInstance </li>
...
public class L2Attackable extends L2NpcInstance
{
     ...
     * <B><U> Data</U> :</B><BR>
     * <BR>
     * <li>attacker : The attaker L2Character concerned by this AggroInfo
     * of this L2Attackable </li>
     * <li>hate : Hate level of this L2Attackable against the attaker
     * L2Character (hate = damage) </li>
     * <li>damage : Number of damages that the attaker L2Character gave to
    ...
    /**
     * This class contains all RewardInfo of the L2Attackable against the
     * any attacker L2Character, based on amount of damage done.<BR>
     * <BR>
     *
     * <B><U> Data</U> :</B><BR>
     * <BR>
     * <li>attacker : The attaker L2Character concerned by this RewardInfo
     * of this L2Attackable </li>
     ...
     * <B><U> Data</U> :</B><BR>
     * <BR>
     * <li>absorber : The attaker L2Character concerned by this
     * AbsorberInfo of this L2Attackable </li>
     *
     */
    ...
    /**
     * This class is used to create item reward lists instead of creating
     * item instances.<BR>
     * <BR>
     */
    ...
    /**
     * The table containing all autoAttackable L2Character in its Aggro
     * Range and L2Character that attacked the L2Attackable This Map is
     * Thread Safe, but Removing Object While Interating Over It Will Result
     * NPE
     */
    ...
    /**
     * Use this to Remove Object from this Map This Should be Synchronized
     * While Interating over This Map - ie u cant interating and removing
     * object at once
     */
    ...
    /**
     * Table containing all Items that a Dwarf can Sweep on this
     * L2Attackable
     */
    ...
    /**
     * The table containing all L2PcInstance that successfuly absorbed the
     * soul of this L2Attackable
     */
     ...
     * L2Attackable (copy skills from template to object and link
    ...
    /**
     * Return the L2Character AI of the L2Attackable and if its null create
     * a new one.<BR>
     * <BR>
     */
    @Override
	 ...
	 * if ((this instanceof L2FortSiegeGuardInstance) && (attacker instanceof
	 * L2FortSiegeGuardInstance))
	 * L2SiegeGuardInstance))
	 * //if((this.getEffect(L2Effect.EffectType.CONFUSION)!=null) &&
	 * (attacker.getEffect(L2Effect.EffectType.CONFUSION)!=null)) return; if
	 * ((this instanceof L2MonsterInstance)&&(attacker instanceof
	 * L2MonsterInstance))
	 * if((this.getEffect(L2Effect.EffectType.CONFUSION)!=null) &&
     ...
     * rewards to L2PetInstance</B></FONT><BR>
     * <BR>
     *
     * @param killer
     ...
     * owner) that hit the L2Attackable and to their Party members.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Get the L2PcInstance owner of the L2SummonInstance (if
     * necessary) and L2Party in progress </li>
     * <li>Calculate the Experience and SP rewards in function of the level
     * difference</li>
     * <li>Add Exp and SP rewards to L2PcInstance (including Summon
     * penalty) and to Party members in the known area of the last attacker
     * </li>
     * <BR>
     * <BR>
     *
	...
	try
	{
	    ...
	    // Manage drop of Special Events created by GM for a defined
	    // period
		...
		for (FastMap.Entry<L2Character, RewardInfo> entry = rewards.head(), end = rewards.tail(); (entry = entry.getNext()) != end;)
		{
		    ...
		    // L2SummonInstance, get Exp Penalty applied for the
		    // current
			...
			{
			    // Calculate the difference of level between
			    // this
			    // attacker (L2PcInstance or L2SummonInstance
			    // owner)
			...
			for (L2PcInstance pl : groupMembers)
			{
				...
				rewards.remove(pl); // Remove the
				// L2PcInstance from the
				// L2Attackable rewards
			    } else
			    {
				...
				// damage
				{
				    ...
				    rewards.remove(summon); // Remove the
				    // summon from
				    // the
				    // L2Attackable
				    // rewards
     ...
     * @param damage
     *                The number of damages given by the attacker
     *                L2Character
     *
     */
     ...
     * @param levelModifier
     *                level modifier in %'s (will be subtracted from drop
     *                chance)
     */
	 ...
	 * Round drop chance dropChance = Math.round(dropChance); // Set our
	 * limits for chance of drop if (dropChance < 1) dropChance = 1; // if
	 * (drop.getItemId() == 57 && dropChance > L2DropData.MAX_CHANCE)
	 * dropChance = L2DropData.MAX_CHANCE; // If item is adena, dont drop
	 * multiple time // Get min and max Item quantity that can be dropped in
	 * one time int minCount = drop.getMinDrop(); int maxCount =
	 * drop.getMaxDrop(); int itemCount = 0; if (itemCount > 0) return new
	 * RewardItem(drop.getItemId(), itemCount); else if (itemCount == 0 &&
	 * Config.DEBUG) _log.fine("Roll produced 0 items to drop..."); return
	 * null;
	 */
    }
    ...
    /**
     * Manage Base, Quests and Special Events drops of L2Attackable (called
     * by calculateRewards).<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * During a Special Event all L2Attackable can drop extra Items. Those
     * extra Items are defined in the table <B>allNpcDateDrops</B> of the
     ...
     * <B><U> Actions</U> : </B><BR>
     * <BR>
     * <li>Manage drop of Special Events created by GM for a defined period
     * </li>
     * <li>Get all possible drops of this L2Attackable from L2NpcTemplate
     * and add it Quest drops</li>
     * <li>For each possible drops (base + quests), calculate which one
     * must be dropped (random) </li>
     ...
     * <li>If the autoLoot mode is actif and if the L2Character that has
     * killed the L2Attackable is a L2PcInstance, give this or these Item(s)
     * to the L2PcInstance that has killed the L2Attackable</li>
     * <li>If the autoLoot mode isn't actif or if the L2Character that has
     * killed the L2Attackable is not a L2PcInstance, add this or these
     * Item(s) in the world as a visible object at the position where mob
     * was last</li>
     * <BR>
     * <BR>
     *
     ...
     * <BR>
     * <li>Get an Item Identifier (random) from the DateDrop Item table of
     * this Event </li>
     ...
     *
     * @param player
     *                The L2Character searched in the _aggroList of the
     *                L2Attackable
     *
     */
    ...
    /**
     * Return true if a Dwarf use Sweep on the L2Attackable and if item can
     * be spoiled.<BR>
     * <BR>
     */
     ...
     *
     * @param attacker
     *                The L2Character who hit on the L2Attackable using the
     *                over-hit enabled skill
     * @param damage
     *                The ammount of damage done by the over-hit enabled
     *                skill on the L2Attackable
     *
     */
    ...
    /**
     * Return the ammount of damage done on the L2Attackable using an
     * over-hit enabled skill.<BR>
     * <BR>
     *
    ...
    /**
     * Adds an attacker that successfully absorbed the soul of this
     * L2Attackable into the _absorbersList.<BR>
     * <BR>
     *
     * params: attacker - a valid L2PcInstance condition - an integer
     * indicating the event when mob dies. This should be: = 0 - "the
     * crystal scatters"; = 1 - "the crystal failed to absorb. nothing
     * happens"; = 2 - "the crystal resonates because you got more than 1
     * crystal on you"; = 3 - "the crystal cannot absorb the soul because
     * the mob level is too low"; = 4 - "the crystal successfuly absorbed
     * the soul";
     */
    ...
    /**
     * Calculate the leveling chance of Soul Crystals based on the attacker
     * that killed this L2Attackable
     *
     * @param attacker
	...
	for (L2PcInstance player : players)
	{
	    ...
	    /*
	     * TODO: Confirm boss chance for crystal level up and for
	     * crystal breaking. It is known that bosses with FULL_PARTY
	     * crystal level ups have 100% success rate, but this is not the
	     * case for the other bosses (one-random or last-hit). While not
	     * confirmed, it is most reasonable that crystals leveled up at
	     * bosses will never break. Also, the chance to level up is
    ...
    /**
     * Calculate the Experience and SP to distribute to attacker
     * (L2PcInstance, L2SummonInstance or L2Party) of the L2Attackable.<BR>
     * <BR>
     *
     * @param diff
     *                The difference of level between attacker
     *                (L2PcInstance, L2SummonInstance or L2Party) and the
     *                L2Attackable
     * @param damage
     ...
     * <BR>
     */
    // This is located here because L2Monster and L2FriendlyMob both extend
    // this
