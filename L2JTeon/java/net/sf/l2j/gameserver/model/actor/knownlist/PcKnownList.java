...
public class PcKnownList extends PlayableKnownList
{
    ...
    // Method - Public
    /**
     * Add a visible L2Object to L2PcInstance _knownObjects and _knownPlayer
     * (if necessary) and send Server-Client Packets needed to inform the
     ...
     * AutoAttackStart to the L2PcInstance </li>
     * <BR>
     * <BR>
     * 
     ...
     * <BR>
     * <li> Send Server-Client Packet NpcInfo/PetItemList (if the
     * L2PcInstance is the owner) to the L2PcInstance </li>
     * <li> Send Server->Client packet MoveToPawn/CharMoveToLocation and
     * AutoAttackStart to the L2PcInstance </li>
     * <BR>
     * <BR>
	...
	} else
	{
		...
		if (otherPlayer.isInBoat())
		{
		    ...
		    /*
		     * if(otherPlayer.getBoat().GetVehicleDeparture() ==
		     * null) { int xboat = otherPlayer.getBoat().getX(); int
		     * yboat= otherPlayer.getBoat().getY(); double modifier =
		     * Math.PI/2; if (yboat == 0) { yboat = 1; } if(yboat <
		     * 0) { modifier = -modifier; } double angleboat =
		     * modifier - Math.atan(xboat/yboat); int xp =
		     * otherPlayer.getX(); int yp = otherPlayer.getY();
		     * modifier = Math.PI/2; if (yp == 0) { yboat = 1; }
		     * if(yboat < 0) { modifier = -modifier; } double anglep =
		     * modifier - Math.atan(yp/xp); double finx =
		     * Math.cos(anglep - angleboat)*Math.sqrt(xp *xp +yp*yp ) +
		     * Math.cos(angleboat)*Math.sqrt(xboat *xboat
		     * +yboat*yboat ); double finy = Math.sin(anglep -
		     * angleboat)*Math.sqrt(xp *xp +yp*yp ) +
		     * Math.sin(angleboat)*Math.sqrt(xboat *xboat
		     * +yboat*yboat );
		     * //otherPlayer.getPosition().setWorldPosition(otherPlayer.getBoat().getX() -
		     * otherPlayer.getInBoatPosition().x,otherPlayer.getBoat().getY() -
		     * otherPlayer.getInBoatPosition().y,otherPlayer.getBoat().getZ()-
		     * otherPlayer.getInBoatPosition().z);
		     * otherPlayer.getPosition().setWorldPosition((int)finx,(int)finy,otherPlayer.getBoat().getZ()-
		     * otherPlayer.getInBoatPosition().z); }
		     */
		} else
		{
    ...
    /**
     * Remove a L2Object from L2PcInstance _knownObjects and _knownPlayer
     * (if necessary) and send Server-Client Packet DeleteObject to the
     * L2PcInstance.<BR>
     * <BR>
     * 
     * @param object
     *                The L2Object to remove from _knownObjects and
     *                _knownPlayer
     * 
     */
    @Override
