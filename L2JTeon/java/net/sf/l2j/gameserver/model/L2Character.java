...
import net.sf.l2j.gameserver.model.actor.instance.L2ClanHallSiegeBossInstance;
...
public abstract class L2Character extends L2Object
{
    ...
    private int _augmentationSkillType = -1; // 1 - physical; 2 - magic; 0 -
     ...
     * have the same number of HP...). All of those properties are stored in
     * a different template for each type of L2Character. Each template is
     * loaded once in the server cache memory (reduce memory use). When a
     * new instance of L2Character is spawned, server just create a link
     * between the instance and the template This link is stored in
     * <B>_template</B><BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     ...
     * slot</li>
     * <li>If L2Character is a L2PcInstance or L2Summon, copy basic
     * Calculator set to object</li>
     * <BR>
     * <BR>
     *
     * @param objectId
	...
	public L2Character(int objectId, L2CharTemplate template)
	{
		...
		if (!(this instanceof L2PcInstance) && !(this instanceof L2MonsterInstance) &&
			...
			!(this instanceof L2ClanHallSiegeBossInstance))
			setIsInvul(true);
	}
     ...
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T REMOVE
     * the object from _allObjects of L2World </B></FONT><BR>
     ...
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * L2PcInstance in the detection area of the L2Character are identified
     * in <B>_knownPlayers</B>. In order to inform other players of state
    ...
    /**
     * Send a packet to the L2Character AND to all L2PcInstance in the
     * radius (max knownlist radius) from the L2Character.<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     ...
     * all other L2PcInstance to inform.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Create the Server->Client packet StatusUpdate with current HP
     * and MP </li>
     * <li>Send the Server->Client packet StatusUpdate with current HP and
     * MP to all L2Character called _statusListener that must be informed of
     * HP/MP updates of this L2Character </li>
     * <BR>
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND
     * CP information</B></FONT><BR>
     * <BR>
     *
     * <B><U> Overriden in </U> :</B><BR>
     * <BR>
     * <li> L2PcInstance : Send current HP,MP and CP to the L2PcInstance and
     * only current HP, MP and Level to all other L2PcInstance of the Party</li>
     * <BR>
     * <BR>
     *
     */
     ...
     * <li>Set the x,y,z position of the L2Object and if necessary modify
     * its _worldRegion</li>
     * <li>Send a Server->Client packet TeleportToLocationt to the
     * L2Character AND to all L2PcInstance in its _KnownPlayers</li>
     ...
     * <BR>
     * <BR>
     * <li>If weapon is a bow, check for arrows, MP and bow re-use delay
     * (if necessary, equip the L2PcInstance with arrows in left hand)</li>
     ...
     * <li>Select the type of attack to start (Simple, Bow, Pole or Dual)
     * and verify if SoulShot are charged then start calculation</li>
     * <li>If the Server->Client packet Attack contains at least 1 hit,
     * send the Server->Client packet Attack to the L2Character AND to all
     * L2PcInstance in the _KnownPlayers of the L2Character</li>
	...
	{
	    /*
	     * ADDED BY nexus - 2006-08-17 As soon as we know that our hit
	     * landed, we must discharge any active soulshots. This must be
	     * done so to avoid unwanted soulshot consumption.
	     */
     ...
     * <li>If the L2Character is a L2PcInstance, Send a Server->Client
     * packet SetupGauge </li>
     * <li>Create a new hit task with Medium priority</li>
     * <li>Calculate and set the disable delay of the bow in function of
     * the Attack Speed</li>
     ...
     *
     * @param attack
     *                Server->Client packet Attack in which the hit will be
     *                added
     * @param target
     *                The L2Character targeted
     * @param sAtk
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Get all visible objects in a spheric area near the L2Character
     * to obtain possible targets </li>
     * <li>If possible target is the L2Character targeted, launch a simple
     * attack against it </li>
     * <li>If possible target isn't the L2Character targeted but is
     * attakable, launch a simple attack against it </li>
     * <BR>
     * <BR>
     *
     * @param attack
    ...
    /**
     * Manage the casting task (casting and interrupt time, re-use delay...)
     * and display the casting bar and animation on client.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     ...
     * <li>Calculate the casting time (base + modifier of MAtkSpd),
     * interrupt time and re-use delay</li>
     ...
     * EnableSkill)</li>
     * <li>Create a task MagicUseTask (that will call method
     * onMagicUseTimer) to launch the Magic Skill at the end of the casting
     * time</li>
     * <BR>
     * <BR>
     *
	 ...
	 */
	if ((this instanceof L2PcInstance) && ((skill.getSkillType() == SkillType.BUFF) || (skill.getSkillType() == SkillType.HOT) || (skill.getSkillType() == SkillType.HEAL) || (skill.getSkillType() == SkillType.COMBATPOINTHEAL) || (skill.getSkillType() == SkillType.HEAL_PERCENT) || (skill.getSkillType() == SkillType.MANAHEAL) || (skill.getSkillType() == SkillType.MANAHEAL_PERCENT) || (skill.getSkillType() == SkillType.BALANCE_LIFE) || (skill.getSkillType() == SkillType.FORCE_BUFF) || (skill.getSkillType() == SkillType.CONT)))
	{
	...
	} else
	{
		...
		//{
		    ...
		    //sendPacket(new ActionFailed());
		    //return;
		//} else if (((L2PcInstance) this).isKoof() && ((((L2PcInstance) target).isKoof() && Config.ENABLE_FACTION_KOOFS_NOOBS && (skill.getSkillType() == SkillType.BUFF)) || (skill.getSkillType() == SkillType.HOT) || (skill.getSkillType() == SkillType.HEAL) || (skill.getSkillType() == SkillType.COMBATPOINTHEAL) || (skill.getSkillType() == SkillType.HEAL_PERCENT) || (skill.getSkillType() == SkillType.MANAHEAL) || (skill.getSkillType() == SkillType.MANAHEAL_PERCENT) || (skill.getSkillType() == SkillType.BALANCE_LIFE) || (skill.getSkillType() == SkillType.CONT)))
		//{
		    //return;
		//}
		...
		//{
		    ...
		    //sendPacket(new ActionFailed());
		    //return;
		//} else if (((L2PcInstance) this).isNoob() && ((((L2PcInstance) target).isNoob() && Config.ENABLE_FACTION_KOOFS_NOOBS && (skill.getSkillType() == SkillType.BUFF)) || (skill.getSkillType() == SkillType.HOT) || (skill.getSkillType() == SkillType.HEAL) || (skill.getSkillType() == SkillType.COMBATPOINTHEAL) || (skill.getSkillType() == SkillType.HEAL_PERCENT) || (skill.getSkillType() == SkillType.MANAHEAL) || (skill.getSkillType() == SkillType.MANAHEAL_PERCENT) || (skill.getSkillType() == SkillType.BALANCE_LIFE) || (skill.getSkillType() == SkillType.CONT)))
		//{
		    //return;
		//}
	    //}
	...
	if (((this instanceof L2Attackable) && ((L2Attackable) this).isUsingShots()))
	{
	    ...
	    _castInterruptTime = (int) (0.70 * _castInterruptTime);
     ...
     * </li>
     * <li>Send the Server->Client packet StatusUpdate with current HP and
     * MP to all other L2PcInstance to inform </li>
     ...
     * Quest Engine </li>
     * <li> L2PcInstance : Apply Death Penalty, Manage gain/loss Karma and
     * Item Drop </li>
     * <BR>
     * <BR>
     *
    ...
    // Property - Public
    /**
     * Return the L2CharacterAI of the L2Character and if its null create a
     * new one.
     */
     ...
     * attackEndTime, fakeDeath, paralyse).
     */
    ...
    /**
     * Return true if the L2Character can't move (stun, root, sleep,
     * overload, paralyzed).
     */
    ...
    /**
     * Return true if the L2Character can be controlled by the player
     * (confused, afraid).
     */
    ...
    /**
     * Set the overloaded status of the L2Character is overloaded (if true,
     * the L2PcInstance can't take more item).
     */
    ...
    /**
     * Set the L2Character movement type to walk and send Server->Client
     * packet ChangeMoveType to all others L2PcInstance.
     */
    ...
    /**
     * The table containing the List of all stacked effect in progress for
     * each Stack group Identifier
     */
    ...
    /**
     * Table EMPTY_EFFECTS shared by all L2Character without effects in
     * progress
     */
    ...
    // Method - Public
    /**
     * Launch and add L2Effect (including Stack Group management) to
     * L2Character and update client magic icone.<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * All active skills effects in progress on the L2Character are
     * identified in ConcurrentHashMap(Integer,L2Effect) <B>_effects</B>.
     * The Integer key of _effects is the L2Skill Identifier that has
     * created the L2Effect.<BR>
     * <BR>
     *
     ...
     * previous in progress. More, some effects belong to the same Stack
     * Group (ex WindWald and Haste Potion). If 2 effects of a same group
     * are used at the same time on a L2Character, only the more efficient
     * (identified by its priority order) will be preserve.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Add the L2Effect to the L2Character _effects</li>
     * <li>If this effect doesn't belong to a Stack Group, add its Funcs to
     * the Calculator set of the L2Character (remove the old one if
     * necessary)</li>
     * <li>If this effect has higher priority in its Stack Group, add its
     * Funcs to the Calculator set of the L2Character (remove previous
     * stacked effect Funcs if necessary)</li>
     * <li>If this effect has NOT higher priority in its Stack Group, set
     * the effect to Not In Use</li>
     ...
     *
     * @param id
     *                The identifier of the stacked effect to add to the
     *                Stack Group
     * @param stackOrder
     ...
     * (Stop L2Effect)</li>
     * <li>If the L2Effect belongs to a not empty Stack Group, replace
     * theses Funcs by next stacked effect Funcs</li>
    ...
    /**
     * Active abnormal effects flags in the binary mask and send
    ...
    /**
     * Active the abnormal effect Confused flag, notify the L2Character AI
     * and send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
    ...
    /**
     * Active the abnormal effect Fake Death flag, notify the L2Character AI
     * and send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
    ...
    /**
     * Active the abnormal effect Fear flag, notify the L2Character AI and
    ...
    /**
     * Active the abnormal effect Muted flag, notify the L2Character AI and
     * send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
    ...
    /**
     * Active the abnormal effect Psychical_Muted flag, notify the
     * L2Character AI and send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
    ...
    /**
     * Active the abnormal effect Root flag, notify the L2Character AI and
     * send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
    ...
    /**
     * Active the abnormal effect Sleep flag, notify the L2Character AI and
     * send Server->Client UserInfo/CharInfo packet.<BR>
     * <BR>
     */
     ...
     * L2Character</li>
     * <li>If Stun succeed, active the abnormal effect Stun flag, notify
     * the L2Character AI and send Server->Client UserInfo/CharInfo packet</li>
     * <li>If Stun NOT succeed, send a system message Failed to the
     * L2PcInstance attacker</li>
     * <BR>
     * <BR>
     *
     */
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Confused abnormal
     * L2Effect from L2Character and update client magic icone </li>
    ...
    /**
     * Stop and remove the L2Effects corresponding to the L2Skill Identifier
     ...
     *
     * @param effectId
     *                The L2Skill Identifier of the L2Effect to remove from
     *                _effects
     *
     */
     ...
     *
     * @param type
     *                The type of effect to stop ((ex : BUFF,
     *                DMG_OVER_TIME...)
     *
     */
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Fake Death abnormal
     * L2Effect from L2Character and update client magic icone </li>
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Fear abnormal L2Effect
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Muted abnormal L2Effect
     * from L2Character and update client magic icone </li>
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Root abnormal L2Effect
     * from L2Character and update client magic icone </li>
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Sleep abnormal L2Effect
     * from L2Character and update client magic icone </li>
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Delete a specified/all (if effect=null) Stun abnormal L2Effect
     * from L2Character and update client magic icone </li>
     ...
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method ONLY UPDATE
     * the client of the player and not clients of all players in the party.</B></FONT><BR>
     * <BR>
     *
     */
     ...
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * In Server->Client packet, each effect is represented by 1 bit of the
     * map (ex : BLEEDING = 0x0001 (bit 1), SLEEP = 0x0080 (bit 8)...). The
     * map is calculated by applying a BINARY OR operation on each effect.<BR>
     * <BR>
     *
     ...
     * <BR>
     * All active skills effects in progress on the L2Character are
     * identified in <B>_effects</B>. The Integer key of _effects is the
     * L2Skill Identifier that has created the effect.<BR>
     * <BR>
     *
     * @return A table containing all active skills effect in progress on
     *         the L2Character
     *
     */
     ...
     * <BR>
     * All active skills effects in progress on the L2Character are
     * identified in <B>_effects</B>.
     *
     * @param index
     *                The L2Skill Identifier of the L2Effect to return from
     *                the _effects
     *
    ...
    /**
     * Return the first L2Effect in progress on the L2Character created by
     * the L2Skill.<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
    ...
    /**
     * Return the first L2Effect in progress on the L2Character
     * corresponding to the Effect Type (ex : BUFF, STUN, ROOT...).<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     ...
     *
     * @param tp
     *                The Effect Type of skills whose effect must be
     *                returned
     *
    ...
    /**
     * This class permit to the L2Character AI to obtain informations and
     * uses L2Character method
     */
     ...
     * modifier of a state (ex : REGENERATE_HP_RATE...). To reduce cache
     * memory use, L2NPCInstances who don't have skills share the same
     * Calculator set called <B>NPC_STD_CALCULATOR</B>.<BR>
     * <BR>
     *
     * That's why, if a L2NPCInstance is under a skill/spell effect that
     * modify one of its state, a copy of the NPC_STD_CALCULATOR must be
     * create in its _calculators before addind new Func object.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>If _calculators is linked to NPC_STD_CALCULATOR, create a copy
     * of NPC_STD_CALCULATOR in _calculators</li>
     * <li>Add the Func object to _calculators</li>
     * <BR>
     * <BR>
     *
     * @param f
     *                The Func object to add to the Calculator corresponding
     ...
     *
     * @param f
     *                The Func object to remove from the Calculator
     *                corresponding to the state affected
     */
    ...
    /**
     * Remove all Func objects with the selected owner from the Calculator
	...
	for (Stats stat : stats)
	{
	     ...
	     * else if (stat == Stats.MAX_HP) { // TODO: self only and add
	     * more stats... if (su == null) su = new
	     * StatusUpdate(getObjectId());
    ...
    /**
     * Return the X destination of the L2Character or the X position if not
     * in movement.<BR>
     * <BR>
     */
    ...
    /**
     * Return the Y destination of the L2Character or the Y position if not
     * in movement.<BR>
     * <BR>
     */
    ...
    /**
     * Return the Z destination of the L2Character or the Z position if not
     * in movement.<BR>
     * <BR>
     */
    ...
    /**
     * Update the position of the L2Character during a movement and return
     * true if the movement is finished.<BR>
     * <BR>
     *
     * <B><U> Concept</U> :</B><BR>
     * <BR>
     * At the beginning of the move action, all properties of the movement
     * are stored in the MoveData object called <B>_move</B> of the
     * L2Character. The position of the start point and of the destination
     * permit to estimated in function of the movement speed the time to
     * achieve the destination.<BR>
     * <BR>
     *
     * When the movement is started (ex : by MovetoLocation), this method
     * will be called each 0.1 sec to estimate and update the L2Character
     * position on the server. Note, that the current server position can
     * differe from the current client position even if each movement is
     * straight foward. That's why, client send regularly a Client->Server
     * ValidatePosition packet to eventually correct the gap on the server.
     * But, it's always the server position that is used in range
     * calculation.<BR>
     * <BR>
     *
     * At the end of the estimated movement time, the L2Character position
     * is automatically set to the destination position even if the movement
     * is not finished.<BR>
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : The current Z position is
     * obtained FROM THE CLIENT by the Client->Server ValidatePosition
     * Packet. But x and y positions must be calculated to avoid that
     * players try to modify their movement speed.</B></FONT><BR>
     * <BR>
     *
     * @param gameTicks
     ...
     * <li>Remove object from _knownObjects and _knownPlayer* of all
     * surrounding L2WorldRegion L2Characters </li>
     * <BR>
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T send
     ...
     * <li>If necessary, add L2Object to _knownObject of the L2Character
     * </li>
     * <li>If necessary, add L2Character to _KnownObject of the L2Object
     * </li>
     ...
     * _statusListener and add it to the new target if it was a L2Character</li>
     * <BR>
     * <BR>
     *
     * @param object
	...
	if (object == null)
	{
	     ...
	     * abortAttack();
	     * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE); if
	     * (this instanceof L2PcInstance) { sendPacket(new
	     * ActionFailed()); SystemMessage sm = new
	     * SystemMessage(SystemMessageId.S1_S2); sm.addString("Attack is
	     * aborted"); sendPacket(sm); } } if (isCastingNow() &&
	     * canAbortCast() && getAI().getCastTarget() == _target) {
	     * abortCast();
	     * getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE); if
	     * (this instanceof L2PcInstance) { sendPacket(new
	     * ActionFailed()); SystemMessage sm = new
	     * SystemMessage(SystemMessageId.S1_S2); sm.addString("Casting
	     * is aborted"); sendPacket(sm); } }
	     */
	}
	_target = object;
    }
     ...
     * achieve the destination.<BR>
     * <BR>
     * All L2Character in movement are identified in <B>movingObjects</B>
     * of GameTimeController that will call the updatePosition method of
     * those L2Character each 0.1s.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Get current position of the L2Character </li>
     * <li>Calculate distance (dx,dy) between current position and
     * destination including offset </li>
     ...
     * </li>
     * <li>Create a task to notify the AI that L2Character arrives at a
     * check point of the movement </li>
     * <BR>
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T send
     ...
     * onIntentionPickUp(L2Object), onIntentionInteract(L2Object) </li>
     * <li> FollowTask </li>
     * <BR>
     * <BR>
     *
     ...
     *                The Y position of the destination
     * @param offset
     *                The size of the interaction area of the L2Character
     *                targeted
     *
     */
    ...
    /**
     * Return the distance between the current position of the L2Character
     * and the target (x,y).<BR>
     * <BR>
     *
     * @param x
     ...
     * @param strictCheck
     *                true if (distance < radius), false if (distance <=
     *                radius)
     * @return true is the L2Character is inside the radius.
     *
     * @see net.sf.l2j.gameserver.model.L2Character.isInsideRadius(int x,
     *      int y, int z, int radius, boolean checkZ, boolean strictCheck)
     */
    ...
    // }
    //
    // // takes care of moving away but distance is 0 so i won't follow
    // problem
    //
    //
    ...
    //
    // if (Config.DEBUG)
    // _log.fine(this.getName() +":: target reached at: x "+getX()+" y
    // "+getY()+
    // " z:" + getZ());
    //
	...
	if (!isAttackAborted())
	{
	    ...
	    // If L2Character target is a L2PcInstance, send a system
	    // message
	    if (target instanceof L2PcInstance)
	    {
		...
		// bow
		{
			 ...
			 * instanceof L2Summon)
			 * ((L2Summon)target).getOwner().sendMessage("Summon
			 * reflected " + reflectedDamage + " damage."); if (this
			 * instanceof L2PcInstance)
			 * ((L2PcInstance)this).sendMessage("Target reflected to
			 * you " + reflectedDamage + " damage."); else if (this
			 * instanceof L2Summon)
			 * ((L2Summon)this).getOwner().sendMessage("Target
			 * reflected to your summon " + reflectedDamage + "
			 * damage.");
			 */
		    }
			...
			{
			    absorbDamage = maxCanAbsorb; // Can't absord
			    // more
			    // than max hp
			}
			if (absorbDamage > 0)
			{
			    ...
			    /*
			     * if (this instanceof L2PcInstance)
			     * ((L2PcInstance)this).sendMessage("You
			     * absorbed " + absorbDamage + " damage."); else
			     * if (this instanceof L2Summon)
			     * ((L2Summon)this).getOwner().sendMessage("Summon
			     * absorbed " + absorbDamage + " damage."); else
			     * if (Config.DEBUG) _log.info(getName() + "
			     * absorbed " + absorbDamage + " damage.");
			     */
			}
		    }
		}
	    ...
	    /*
	     * COMMENTED OUT BY nexus - 2006-08-17 We must not discharge the
	     * soulshouts at the onHitTimer method, as this can cause
	     * unwanted soulshout consumption if the attacker recharges the
	     * soulshot right after an attack request but before his hit
	     * actually lands on the target. The soulshot discharging has
	     * been moved to the doAttack method: As soon as we know that we
	     * didn't missed the hit there, then we must discharge any
	     * charged soulshots.
	     */
	    /*
	     * L2ItemInstance weapon = getActiveWeaponInstance(); if (!miss) {
	     * if (this instanceof L2Summon && !(this instanceof
    ...
    /**
     * Break an attack and send Server->Client ActionFailed packet and a
     * System Message to the L2Character.<BR>
     * <BR>
     */
     ...
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>If L2Character or target is in a town area, send a system
     ...
     * </li>
     * <li>If L2Character is a L2ArtefactInstance, send a Server->Client
     * packet ActionFailed </li>
     * <li>Send a Server->Client packet MyTargetSelected to start attack
     * and Notify AI with AI_INTENTION_ATTACK </li>
     * <BR>
     * <BR>
     *
     * @param player
     ...
     *
     * @param skillId
     *                The identifier of the L2Skill whose level must be
     *                returned
     *
     ...
     *
     * @param preferSkill
     *                If != 0 the given skill Id will be removed instead of
     *                first
     */
    ...
    /**
     * Manage the magic skill launching task (MP, HP, Item consummation...)
     * and display the magic skill animation on client.<BR>
     * <BR>
     *
     * <B><U> Actions</U> :</B><BR>
     * <BR>
     * <li>Send a Server->Client packet MagicSkillLaunched (to display
     * magic skill animation) to all L2PcInstance of L2Charcater
     * _knownPlayers</li>
     * <li>Consumme MP, HP and Item if necessary</li>
     * <li>Send a Server->Client packet StatusUpdate with MP modification
     * to the L2PcInstance</li>
     ...
     * <BR>
     *
     * <FONT COLOR=#FF0000><B> <U>Caution</U> : A magic skill casting MUST
     * BE in progress</B></FONT><BR>
     * <BR>
     *
     * @param skill
     *                The L2Skill to use
	...
	try
	{
	    ...
	    // StatusUpdate with current HP and MP to all other L2PcInstance
	    // to inform
    ...
    /**
     * Launch the magic skill and calculate its effects on each target
     * contained in the targets table.<BR>
     * <BR>
     *
     * @param skill
     *                The L2Skill to use
	...
	try
	{
	    // Do initial checkings for skills and set pvp flag/draw aggro
	    // when
	    // needed
		...
		{
		    // Set some values inside target's instance for later
		    // use
		    L2Character player = (L2Character) target;
		    L2Weapon activeWeapon = getActiveWeaponItem();
		    // Launch weapon Special ability skill effect if
		    // available
