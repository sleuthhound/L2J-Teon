...
{
    protected static Logger _log = Logger.getLogger(AutoAnnouncementHandler.class.getName());
    private static AutoAnnouncementHandler _instance;
    private static final long DEFAULT_ANNOUNCEMENT_DELAY = 180000; // 3
    // mins
    // by
    // default
	...
	try
	{
	    ...
	    statement.close();
	    if (Config.DEBUG)
		_log.config("AutoAnnouncementHandler: Loaded " + numLoaded + " announcement from the database.");
	} catch (Exception e)
	{
	} finally
	{
	...
	replyMSG.append("<td width=180><center>OneoDevTeam AutoAnnounce</center></td>");
	replyMSG.append("<td width=40></td>");
     ...
     *                announcementTexts
     * @param int
     *                announcementDelay (-1 = default delay)
	...
	{
	    _log.log(Level.SEVERE, "Could not insert autoAnnouncment into DB: Reason: " + "Duplicate Id");
	} finally
	{
	    try
	    {
	...
	{
	    _log.log(Level.SEVERE, "Could not delete auto announcement in DB:", e);
	} finally
	{
	    try
	    {
     ...
     * specified, and cancels the scheduled chat task if necessary.
     * 
     * @param boolean
     *                isActive
     */
