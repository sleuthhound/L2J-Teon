 ...
 * @author Maxi
 */
public class CHSiegeGuardManager
{
    private static Logger _log = Logger.getLogger(CHSiegeGuardManager.class.getName());
    // =========================================================
    // Data Field
    ...
    // =========================================================
    // Constructor
    public CHSiegeGuardManager(ClanHall clanHall)
    {
	_clanHall = clanHall;
    }
	...
	try
	{
	    ...
	    PreparedStatement statement = con.prepareStatement("SELECT * FROM clanhall_siege_guards Where clanHallId = ? And isHired = ?");
	    statement.setInt(1, getClanHall().getClanHallId());
	    if (getClanHall().getOwnerId() > 0) // If clanhall is owned by a clan,
	...
	{
	    _log.warning("Error loading siege guard for clanHall " + getClanHall().getName() + ":" + e1);
	} finally
	{
	    try
	    {
	...
	{
	    con = L2DatabaseFactory.getInstance().getConnection();
	    PreparedStatement statement = con.prepareStatement("Insert Into clanhall_siege_guards (clanHallId, npcId, x, y, z, heading, respawnDelay, isHired) Values (?, ?, ?, ?, ?, ?, ?, ?)");
	    statement.setInt(1, getClanHall().getClanHallId());
	...
	{
	    _log.warning("Error adding siege guard for clanHall " + getClanHall().getName() + ":" + e1);
	} finally
	{
	    try
	    {
