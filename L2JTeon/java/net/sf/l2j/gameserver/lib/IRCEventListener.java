 ...
 * <p>
 * The <code>IRCEventListener</code> is used by the <code>IRCConnection.addEventListener(IRCEventListener)</code> method to add a listener which listens to the connection for incoming IRC events like <code>PRIVMSG</code>s or numeric replies.
 * <p>
 * Supported events:
 * <ul>
 * <li>Connect</li>
...
public interface IRCEventListener extends EventListener, IRCConstants
{
	 ...
	 * This happens between the connection is opened with a socket and the connection is registered: The client sends his information to the server (nickname, username). The server says hello to you by sending you some <code>NOTICE</code>s. And if your nickname is invalid or in use or anything else is wrong with your nickname, it asks you for a new one.
	 */
	 ...
	 * @param modeParser
	 *            The <code>IRCModeParser</code> object which contains the parsed information about the modes which are changed.
	 */
