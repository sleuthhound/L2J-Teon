...
public class IRCConnection extends Thread
{
	...
	/**
	 * This is like a UNIX-runlevel. Its value indicates the level of the <code>IRCConnection</code> object. <code>0</code> means that the object has not yet been connected, <code>1</code> means that it's connected but not registered, <code>2</code> means that it's connected and registered but still waiting to receive the nickname the first time, <code>3</code> means that it's connected and
	 * registered, and <code>-1</code> means that it was connected but is disconnected. Therefore the defaultvalue is <code>0</code>.
	 */
	...
	/**
	 * The <code>int[]</code> contains all ports to which we are going to try to connect. This can be a portrange from port 6667 to 6669, for example.
	 */
	protected int[] ports;
	/**
	...
	/**
	 * The <code>String</code> contains the name of the character encoding used to talk to the server. This can be ISO-8859-1 or UTF-8 for example. The default is ISO-8859-1.
	 */
	...
	/**
	 * This <code>int</code> is the connection's timeout in milliseconds. It's used in the <code>Socket.setSoTimeout</code> method. The default is <code>1000 * 60 * 15</code> millis which are 15 minutes.
	 */
	 ...
	 * It means, that if the server asks with PING for the ping, the PONG is automatically sent. Default is automatic PONG enabled (<code>true</code>).
	 */
	 ...
	 * The difference to the other constructor is, that it transmits the ports in an <code>int[]</code>. Thus, also ports like 1024, 2048, 6667 and 6669 can be selected.<br />
	 * <br />
	 ...
	 * @param pass
	 *            The password of the IRC server. If your server isn't secured by a password (that's normal), use <code>null</code> or <code>""</code>.
	 * @param nick
	 ...
	 *             If the <code>host</code> or <code>ports</code> is <code>null</code> or <code>ports</code>' length is <code>0</code>.
	 * @see #connect()
	 */
	...
	/**
	 * Creates a new IRC connection. <br />
	 * The difference to the other constructor is, that it transmits the ports as two <code>int</code>s. Thus, only a portrange from port <code>x</code> to port <code>y</code> like from port 6667 to 6669 can be selected.<br />
	 * <br />
	 * The constructor prepares a new IRC connection which can be really started by invoking the <code>connect</code> method. Before invoking it, you should set the <code>IRCEventListener</code> and other settings.<br />
	 * Note that you do not need to set a password to connect to the large public IRC networks like QuakeNet, EFNet etc. To use no password in your IRC connection, use <code>""</code> or <code>null</code> for the password argument in the constructor.
	 *
	...
	// ------------------------------
	/**
	 * The <code>Thread</code> is started by the <code>connect</code> method. It's task is to receive strings from the IRC server and hand them over to the <code>get</code> method.
	 */
	@Override
	public void run()
	{
