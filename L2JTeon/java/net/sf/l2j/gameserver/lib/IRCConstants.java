...
public interface IRCConstants
{
     ...
     * (b) not a chanop (or mode +v) on a channel which has mode +m set and
     ...
     * errors that are returned when an invalid use of "PRIVMSG
     * $&lt;server&gt;" or "PRIVMSG #&lt;host&gt;" is attempted.
     */
     ...
     * which do not fall in the defined set. See section x.x.x for details
     * on valid nicknames.
     */
    ...
    /**
     * Format: "Permission Denied- You're not an IRC operator". Any command
     * requiring operator privileges to operate must return this error to
     ...
     * reply string is composed as follows:<br />
     * &lt;reply&gt; ::= &lt;nick&gt;['*'] '='
     * &lt;'+'|'-'&gt;&lt;hostname&gt; <br />
     ...
     * These replies are used with the AWAY command (if allowed). RPL_AWAY
    ...
    /**
     * Format: "&lt;nick&gt; &lt;user&gt; &lt;host&gt; * :&lt;real
     * name&gt;". <br />
     * Replies 311 - 313, 317 - 319 are all replies generated in response to
     * a WHOIS message. Given that there are enough parameters present, the
     * answering server must either formulate a reply out of the above
     ...
     * replies with data and end of the server's response to a LIST command.
     * If there are no channels available to return, only the start and end
     * reply must be sent.
     */
     ...
     * This is part of /WHOIS on many servers which provide bots to
     * authenticate.
     */
     ...
     * two replies is sent. If the topic is set, RPL_TOPIC is sent back else
     ...
     * server is running in "debug mode". <br />
     ...
     * with a WHO message, a RPL_ENDOFWHO must be sent after processing each
     ...
     * and RPL_ENDOFNAMES is sent by the server back to the client. If there
     * is no channel found as in the query, then only RPL_ENDOFNAMES is
     * returned. The exception to this is when a NAMES message is sent with
     * no parameters and all visible channels and contents are sent back in
     * a series of RPL_NAMEREPLY messages with a RPL_ENDOFNAMES to mark the
     * end.
     */
    ...
    /**
     * Format: "&lt;mask&gt; &lt;server&gt; &lt;hopcount&gt; &lt;server
     * info&gt;". <br />
     ...
     * characters, using RPL_MOTD format replies. These should be surrounded
     * by a RPL_MOTDSTART (before the RPL_MOTDs) and an RPL_ENDOFMOTD
     * (after).
     */
     ...
     * the RPL_TIME format above. The string showing the time need only
     ...
     * list of RPL_TRACELINKs sent in response to a TRACE command traversing
     * the IRC network should reflect the actual connectivity of the servers
     * themselves along that path. RPL_TRACENEWTYPE is to be used for any
     ...
     * &lt;sent bytes&gt; &lt;received messages&gt; &lt;received bytes&gt;
     * &lt;time open&gt;".
     */
     ...
     * from RPL_LUSERCLIENT, RPL_LUSEROP, RPL_USERUNKNOWN, RPL_LUSERCHANNELS
     * and RPL_LUSERME. When replying, a server must send back
     * RPL_LUSERCLIENT and RPL_LUSERME. The other replies are only sent back
     * if a non-zero count is found for them.
     */
    ...
    /**
     * This is part of the mIRC code and shows that a color-code starts /
     * ends. Here it is as the ASCII decimal int 3.
     */
    public static final char COLOR_INDICATOR = 3; // ASCII code
    /**
     * This is part of the mIRC code and shows that bold starts / ends. Here
     * it is as the ASCII decimal int 32.
     */
    public static final char BOLD_INDICATOR = 31; // ASCII code
    /**
     * This is part of the mIRC code and shows that bold starts / ends. Here
     * it is as the ASCII decimal int 2.
     */
    public static final char UNDERLINE_INDICATOR = 2; // ASCII code
    /**
     * This is part of the mIRC code and shows that bold, underline and
     * colors end. Here it is as the ASCII decimal int 15.
     */
    public static final char COLOR_END_INDICATOR = 15; // ASCII code
    /**
     * This is part of the mIRC code and indicates that the client's colors
     * are reversed (background -&gt; foreground and foreground -&gt;
     * background). Here it is as the ASCII decimal int 1.
     */
    public static final char COLOR_REVERSE_INDICATOR = 22; // ASCII code
    /**
     * This is part of the mIRC code and shows that a PRIVMSG is an ACTION (<code>/me</code>).
