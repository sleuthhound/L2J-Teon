...
public final class RequestBuyItem extends L2GameClientPacket
{
	...
	for (int i = 0; i < _count; i++)
	{
	     ...
	     * null || li.getCount() < cnt) { cnt = li.getCount(); if (cnt <=
	     * 0) { items.remove(i); continue; }
	     * items.get(i).setCount((int)cnt); } price =
	     * li.getPriceToSell(); // lease holder sells the item weight =
	     * li.getItem().getWeight(); }
	     */
	    if (price < 0)
	    {
	...
	for (int i = 0; i < _count; i++)
	{
	     ...
	     * item.getCount()); li.updateDatabase(); price =
	     * item.getCount() + li.getPriceToSell(); L2ItemInstance la =
