...
public class MTRandom extends Random
{
     ...
     * code. Furthermore the setSeed() method, which must take a 64 bit long
     * value, will be limited to using only the lower 32 bits of the seed to
     * facilitate seamless migration of existing C code into Java where
     ...
     * given 64 bit seed value. For a better random number sequence this
     * seed value should contain as much entropy as possible.
     * 
     * @param seed
     ...
     * byte array. All the data will be used to initialise this instance.
     * 
     * @param buf
    ...
    /**
     * This method resets the state of this instance using the 64 bits of
     * seed data provided. Note that if the same seed data is passed to two
     * different instances of MTRandom (both of which share the same
     * compatibility state) then the sequence of numbers generated by both
     * instances will be identical.
     * <p>
     * If this instance was initialised in 'compatibility' mode then this
     * method will only use the lower 32 bits of any seed value passed in
     * and will match the behaviour of the original C code exactly with
     * respect to state initialisation.
     * 
     * @param seed
     ...
     * this seed data to allow the pseudo random sequence to be restarted
     * then it would be more efficient to use the "pack()" method to convert
     * it into an integer array first and then use that to re-seed the
    ...
    /**
     * This method resets the state of this instance using the integer array
     * of seed data provided. This is the canonical way of resetting the
     ...
     * <p>
     * Note that where the number of bits requested is not equal to 32 then
     * bits will simply be masked out from the top of the returned integer
     * value. That is to say that:
     * 
     * <pre>
     * mt.setSeed(12345);
     ...
     * seed data is to be used to repeatedly re-seed the random number
     * sequence. By packing the byte array into an integer array first,
     * using this method, and then invoking setSeed() with that; it removes
     * the need to re-pack the byte array each time setSeed() is called.
     * <p>
     ...
     * 
     * <p>
     * Note that this method will not complain if the given byte array is
     * empty and will produce an empty integer array, but the setSeed()
     * method will throw an exception if the empty integer array is passed
     * to it.
     * 
     * @param buf
