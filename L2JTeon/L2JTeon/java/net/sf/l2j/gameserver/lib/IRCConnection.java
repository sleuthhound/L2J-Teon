...
public class IRCConnection extends Thread
{
     ...
     * <code>IRCConnection</code> object. <code>0</code> means that the
     * object has not yet been connected, <code>1</code> means that it's
     * connected but not registered, <code>2</code> means that it's
     * connected and registered but still waiting to receive the nickname
     * the first time, <code>3</code> means that it's connected and
     * registered, and <code>-1</code> means that it was connected but is
     * disconnected. Therefore the defaultvalue is <code>0</code>.
     */
    ...
    /**
     * The <code>int[]</code> contains all ports to which we are going to
     * try to connect. This can be a portrange from port 6667 to 6669, for
     * example.
     */
    protected int[] ports;
    /**
     * The <code>BufferedReader</code> receives Strings from the IRC
     * server.
     */
    ...
    /**
     * The <code>String</code> contains the name of the character encoding
     * used to talk to the server. This can be ISO-8859-1 or UTF-8 for
     * example. The default is ISO-8859-1.
     */
    ...
    /**
     * This <code>int</code> is the connection's timeout in milliseconds.
     * It's used in the <code>Socket.setSoTimeout</code> method. The
     * default is <code>1000 * 60 * 15</code> millis which are 15 minutes.
     */
    private int timeout = 1000 * 60 * 15;
    /**
     * This <code>boolean</code> stands for enabled (<code>true</code>)
     * or disabled (<code>false</code>) ColorCodes.<br />
     * Default is enabled (<code>false</code>).
     */
    private boolean colorsEnabled = false;
    /**
     * This <code>boolean</code> stands for enabled or disabled automatic
     * PING? PONG! support. <br />
     ...
     * The difference to the other constructor is, that it transmits the
     * ports in an <code>int[]</code>. Thus, also ports like 1024, 2048,
     * 6667 and 6669 can be selected.<br />
     * <br />
     * The constructor prepares a new IRC connection which can be really
     * started by invoking the <code>connect</code> method. Before
     * invoking it, you should set the <code>IRCEventListener</code> and
     * other settings.<br />
     * Note that you do not need to set a password to connect to the large
     * public IRC networks like QuakeNet, EFNet etc. To use no password in
     * your IRC connection, use <code>""</code> or <code>null</code> for
     * the password argument in the constructor.
     * 
     * @param host
     ...
     * @param nick
     *                The nickname for the connection. Is used to register
     *                the connection.
     * @param username
     ...
     *                 <code>null</code> or <code>ports</code>' length
     *                 is <code>0</code>.
     * @see #connect()
     */
     ...
     * Creates a new IRC connection. <br />
     * The difference to the other constructor is, that it transmits the
     * ports as two <code>int</code>s. Thus, only a portrange from port
     * <code>x</code> to port <code>y</code> like from port 6667 to 6669
     * can be selected.<br />
     * <br />
     * The constructor prepares a new IRC connection which can be really
     * started by invoking the <code>connect</code> method. Before
     ...
     *                The hostname of the server we want to connect to.
     * @param portMin
     *                The beginning of the port range we are going to
     *                connect to.
     * @param portMax
     *                The ending of the port range we are going to connect
     *                to.
     * @param pass
     *                The password of the IRC server. If your server isn't
     *                secured by a password (that's normal), use
    ...
    // ------------------------------
    /**
     * Converts a portrange which starts with a given <code>int</code> and
     * ends with a given <code>int</code> into an array which contains all
     * <code>int</code>s from the beginning to the ending (including
     ...
     * doesn't do that!<br />
     * It tries all set ports until one is open. If all ports fail it throws
     * an <code>IOException</code>.<br />
     * You can invoke <code>connect</code> only one time.
     * 
     * @throws IOException
     *                 If an I/O error occurs.
     * @throws SocketException
     *                 If the <code>connect</code> method was already
     *                 invoked.
     * @see #isConnected()
     * @see #doQuit()
     * @see #doQuit(String)
    ...
    // ------------------------------
    /**
     * Invoked by the <code>connect</code> method, this method prepares
     * the connection. <br />
     * It initializes the class-vars for the inputstream and the
     * outputstream of the socket, starts the registration of at the IRC
     * server by calling <code>register()</code> and starts the receiving
     * of lines from the server by starting the thread with the
     * <code>start</code> method.<br />
     * <br />
     * This method must be protected, because it is used by extending
     * classes, which override the <code>connect</code> method.
     * 
     * @param s
     ...
     * In fact, it sends a password (if set, else nothing), the nickname and
     * the user, the realname and the host which we're connecting to.<br />
     ...
     * must do it. For most purposes, there are <code>do*</code> methods
     * (like <code>doJoin</code>). A carriage return line feed (<code>\r\n</code>)
     * is appended automatically.
     * 
     * @param line
     *                The line which should be send to the server without
     *                the trailing carriage return line feed (<code>\r\n</code>).
     */
     ...
     * <code>IRCParser</code> class. Then it controls the command and
     * fires events through the <code>IRCEventListener</code>.<br />
     * 
     * @param line
	...
	int reply; // 3-digit reply will be parsed in the later
	// if-condition
	...
	{ // PART
	    ...
	    // not logic: "PART :#zentrum" is without
     ...
     * It does *NOT* send the proper IRC command <code>QUIT</code>. You
     * should always use the <code>doQuit</code> methods or
     * <code>send("QUIT")</code> instead of this method. <br />
     * You should use this method to close down the connection only when the
     * IRC server doesn't react to the <code>QUIT</code> command.
     * 
     * @see #connect()
     * @see #doQuit
     * @see #doQuit(String)
    ...
    // ------------------------------
    /**
     * Adds a new {@link org.schwering.irc.lib.IRCEventListener} which
     * listens for actions coming from the IRC server.
     * 
     * @param l
     *                An instance of the
     *                {@link org.schwering.irc.lib.IRCEventListener}
     *                interface.
     * @throws IllegalArgumentException
     ...
     * {@link org.schwering.irc.lib.IRCEventListener} from the
     * listener-vector.
     * 
     * @param l
     *                An instance of the
     ...
     * 
     * @param pong
     *                <code>true</code> to enable automatic
     *                <code>PONG</code> reply, <code>false</code> makes
     *                the class fire <code>onPing</code> events.
     */
    ...
    // ------------------------------
    /**
     * Changes the character encoding used to talk to the server. This can
     * be ISO-8859-1 or UTF-8 for example. This property must be set before
     * a call to the <code>connect()</code> method.
     * 
     * @param encoding
     */
     ...
     * The default is <code>1000 * 60 15</code> millis which are 15
     * minutes.
     */
     ...
     * If <code>connect</code> wasn't called yet, it returns
     * <code>false</code>.
     * 
    ...
    // ------------------------------
    /**
     * Returns all ports to which the <code>IRCConnection</code> is going
     * to try or has tried to connect to.
     * 
    ...
    // ------------------------------
    /**
     * Returns the port to which the <code>IRCConnection</code> connected,
     * or <code>0</code> if the connection failed or wasn't tried yet.
     * 
     * @return The port to which the <code>IRCConnection</code>, or
     *         <code>0</code> if the connection failed or wasn't tried
     *         yet.
     */
    public int getPort()
    {
     ...
     * If an error occurs, which is never the case, <code>-1</code> is
     * returned.
     * 
     * @return The timeout.
     */
    ...
    // ------------------------------
    /**
     * Generates a <code>String</code> with some information about the
     * instance of <code>IRCConnection</code>. Its format is: <code>
    ...
    {
	return getClass().getName() + "[" + host + "," + getPort() + "," + username + "," + nick + "," + realname + "," + pass + "," + isConnected() + "]";
    }
     ...
     * 
     * @param chan
     *                The channel the <code>NAMES</code> command is
     *                refering to.
     */
     ...
     * 
     * @param chan
     *                The channel the <code>MODE</code> request is
     *                refering to.
     */
     ...
     * The first argument is a nickname (user-mode) or a channel
     * (channel-mode). <code>String mode</code> must contain the operators
     * (+/-), the modes (o/v/i/k/l/p/s/w) and the possibly values
     * (nicks/banmask/limit/key).
     * 
     * @param target
     *                The nickname or channel of the user whose modes will
     *                be changed.
     * @param mode
     *                The new modes.
     */
     ...
     * 
     * @param ping
     *                The ping which was received in <code>onPing</code>.
     *                It's a <code>String</code>, because sometimes on
     *                some networks the server-hostname (for example
     *                splatterworld.quakenet.org) is given as parameter
     *                which would throw an Exception if we gave the ping as
     *                long.
     */
    ...
    // ------------------------------
    /**
     * Quits from the IRC server. Calls the <code>disconnect</code>-method
     * which does the work actually.
     * 
     * @see #isConnected()
     * @see #connect()
     * @see #doQuit(String)
    ...
    // ------------------------------
    /**
     * Requests information about users matching the given criteric, for
     * example a channel they are on.
     * 
     * @param criteric
    ...
    // ------------------------------
    /**
     * Requires host-information about up to 5 users which must be listed
     * and divided by spaces.
     * 
     * @param nick
     *                The nickname of the user the query is refering to.
     */
