...
public class CrestCache
{
	...
	public static void load()
	{
		...
		File[] files = new File(Config.DATAPACK_ROOT, CRESTS_DIR).listFiles(bmpFilter);
		if (files == null)
			files = new File[0];
	...
	public static void convertOldPledgeFiles()
	{
		...
		final File[] files = new File(Config.DATAPACK_ROOT, CRESTS_DIR).listFiles(oldPledgeFilter);
		for (File file : files)
		{
			...
			if (clan != null)
			{
				...
				file.renameTo(new File(Config.DATAPACK_ROOT, CRESTS_DIR + "Crest_" + newId + ".bmp"));
	...
	public static void removeCrest(CrestType crestType, int id)
	{
		...
		File crestFile = new File(Config.DATAPACK_ROOT, CRESTS_DIR + crestDirPrefix + id + ".bmp");
		try
		{
			crestFile.delete();
		}
	...
	{
		File crestFile = new File(Config.DATAPACK_ROOT, CRESTS_DIR + crestType.getDirPrefix() + newId + ".bmp");
