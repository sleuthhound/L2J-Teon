...
public abstract class L2ZoneType
{
	...
	{
		_id = id;
		_characterList = new FastList<L2Character>().shared();
	...
	/**
	 * @param allSummonTypes : if set to true, return pets aswell.
	 * @return all playables within this zone.
	 */
	public List<L2Playable> getPlayableInside(boolean allSummonTypes)
	{
		List<L2Playable> playables = new ArrayList<>();
		
		for (L2Character temp : _characterList)
		{
			if (temp != null && temp instanceof L2Playable)
			{
				if (temp instanceof L2PetInstance && !allSummonTypes)
					continue;
				
				playables.add((L2Playable) temp);
			}
		}
