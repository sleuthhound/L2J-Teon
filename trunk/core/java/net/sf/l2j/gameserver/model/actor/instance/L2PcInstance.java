...
public final class L2PcInstance extends L2Playable
{
	 ...
	 * @param npcId The NPC id to make checks on.
	 * @return An array of QuestState containing that L2Npc.
	 */
	 ...
	 * @param unequipped The item which needs to be decharged.
	 */
	...
	protected void doPickupItem(L2Object object)
	{
		...
		if (!(object instanceof L2ItemInstance))
		{
			...
			_log.warning(this + " have tried to pickup a wrong target: " + getTarget());
			return;
		}
	...
	public void updatePvPStatus(L2Character target)
	{
		...
		if ((!isInsideZone(ZONE_PVP) || !player.isInsideZone(ZONE_PVP)) && player.getKarma() == 0)
		{
			if (checkIfPvP(player))
				PvpFlagTaskManager.getInstance().addPvpFlagTask(this, System.currentTimeMillis() + Config.PVP_PVP_TIME);
			else
				PvpFlagTaskManager.getInstance().addPvpFlagTask(this, System.currentTimeMillis() + Config.PVP_NORMAL_TIME);
	...
	{
		if (!isDead() && !WaterTaskManager.getInstance().isInWater(this))
		{
	...
	{
		if ((getKarma() > 0 || Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE) && !(killer instanceof L2PcInstance) && !(isGM()) && !(getCharmOfLuck() && killer.isRaid()) && !isPhoenixBlessed() && !(isInsideZone(L2Character.ZONE_PVP) || isInsideZone(L2Character.ZONE_SIEGE)))
