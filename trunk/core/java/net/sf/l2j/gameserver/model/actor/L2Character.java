...
import net.sf.l2j.gameserver.util.L2TIntObjectHashMap;
...
public abstract class L2Character extends L2Object
{
	 ...
	 * <BR>
	 * <li>If L2Character is a L2Npc, copy skills from template to object</li> <li>If L2Character is a L2Npc, link _calculators to NPC_STD_CALCULATOR</li><BR>
	 * <BR>
	 * <li>If L2Character is NOT a L2Npc, create an empty _skills slot</li> <li>If L2Character is a L2PcInstance or L2Summon, copy basic Calculator set to object</li><BR>
	 * <BR>
	 ...
	 * time and re-use delay</li> <li>Send a Server->Client packet MagicSkillUse (to diplay casting animation), a packet SetupGauge (to display casting bar) and a system message</li> <li>Disable all skills during the casting time (create a task EnableAllSkills)</li> <li>Disable the skill during the
	...
	private void beginCast(L2Skill skill, boolean simultaneously)
	{
		...
		if (this instanceof L2Playable)
		{
			if (skill.useSoulShot())
				getActingPlayer().rechargeAutoSoulShot(true, false, this instanceof L2Summon);
			else if (skill.useSpiritShot())
				getActingPlayer().rechargeAutoSoulShot(false, true, this instanceof L2Summon);
		}
	 ...
	 * <B><U> Overridden in </U> :</B><BR>
	 * <BR>
	 * <li>L2Npc : Create a DecayTask to remove the corpse of the L2Npc after 7 seconds</li> <li>L2Attackable : Distribute rewards (EXP, SP, Drops...) and notify Quest Engine</li> <li>L2PcInstance : Apply Death Penalty, Manage gain/loss Karma and Item Drop</li><BR>
	 * <BR>
	 ...
	 * <B><U> Concept</U> :</B><BR>
	 * <BR>
	 * A L2Character owns a table of Calculators called <B>_calculators</B>. Each Calculator (a calculator per state) own a table of Func object. A Func object is a mathematic function that permit to calculate the modifier of a state (ex : REGENERATE_HP_RATE...). To reduce cache memory use, L2Npcs
	 * who don't have skills share the same Calculator set called <B>NPC_STD_CALCULATOR</B>.<BR>
	 * <BR>
	 * That's why, if a L2Npc is under a skill/spell effect that modify one of its state, a copy of the NPC_STD_CALCULATOR must be create in its _calculators before addind new Func object.<BR>
	 * <BR>
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 ...
	 * <BR>
	 * <li>If L2Character is a L2Npc and _calculators is equal to NPC_STD_CALCULATOR, free cache memory and just create a link on NPC_STD_CALCULATOR in _calculators</li><BR>
	 * <BR>
	 * <B><U> Example of use </U> :</B><BR>
	 * <BR>
	...
	protected void moveToLocation(int x, int y, int z, int offset)
	{
			...
			&& !(this instanceof L2NpcWalkerInstance)) // npc walkers not checked
		{
			...
			if ((Config.GEODATA == 2 && !(this instanceof L2Attackable && ((L2Attackable) this).isReturningToSpawnPoint())) || (this instanceof L2PcInstance && !(isInVehicle && distance > 1500)) || (this instanceof L2Summon && !(getAI().getIntention() == AI_INTENTION_FOLLOW)) // assuming
																																																																						// intention_follow
																																																																						// only when
																																																																						// following owner
	...
	/**
	 * @param attacker The attacker to test.
	...
	public static boolean isInsidePeaceZone(L2Object attacker, L2Object target)
	{
		...
		if (Config.KARMA_PLAYER_CAN_BE_KILLED_IN_PZ)
		{
	...
	/**
	 * Uncharge the character's soulshot, if the skill allows it.
	 * @param isStaticReuse
	 */
	public void ssUncharge(boolean isStaticReuse)
	{
		if (isStaticReuse)
			return;
		...
		else if (this instanceof L2Summon) // If is not player, check for summon.
			((L2Summon) this).setChargedSoulShot(L2ItemInstance.CHARGED_NONE);
		else if (this instanceof L2Npc)
	...
	/**
	 * Uncharge the character's spiritshot, if the skill allows it.
	 * @param isStaticReuse
	 */
	public void spsUncharge(boolean isStaticReuse)
	{
		if (isStaticReuse)
			return;
	...
	{
		final L2ItemInstance weapon = getActiveWeaponInstance();
		if (this instanceof L2PcInstance && !skill.isMagic() && weapon != null && weapon.getChargedSoulshot() == L2ItemInstance.CHARGED_SOULSHOT)
			return true;
		
		if (this instanceof L2Npc && ((L2Npc) this)._soulshotcharged)
			return true;
		
		return this instanceof L2Summon && ((L2Summon) this).getChargedSoulShot() == L2ItemInstance.CHARGED_SOULSHOT;
	}
	
	public boolean isSpiritshotCharged(L2Skill skill)
	{
		final L2ItemInstance weapon = getActiveWeaponInstance();
		if (this instanceof L2PcInstance && skill.isMagic() && weapon != null && weapon.getChargedSpiritshot() == L2ItemInstance.CHARGED_SPIRITSHOT)
			return true;
		
		if (this instanceof L2Npc && ((L2Npc) this)._spiritshotcharged)
			return true;
		
		return this instanceof L2Summon && ((L2Summon) this).getChargedSpiritShot() == L2ItemInstance.CHARGED_SPIRITSHOT;
	}
	
	public boolean isBlessedSpiritshotCharged(L2Skill skill)
	{
		final L2ItemInstance weaponInst = getActiveWeaponInstance();
		if (this instanceof L2PcInstance && skill.isMagic() && weaponInst != null && weaponInst.getChargedSpiritshot() == L2ItemInstance.CHARGED_BLESSED_SPIRITSHOT)
			return true;
		
		return this instanceof L2Summon && ((L2Summon) this).getChargedSpiritShot() == L2ItemInstance.CHARGED_BLESSED_SPIRITSHOT;
	}
}