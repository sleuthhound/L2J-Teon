...
public final class QuestState
{
	...
	/**
	 * Insert (or Update) in the database variables that need to stay persistant for this player after a reboot. This function is for storage of values that do not related to a specific quest but are global for all quests. For example, player's can get only once the adena and XP reward for the first
	 * class quests, but they can make more than one first class quest.
	...
	public void rewardItems(int itemId, int count)
	{
		...
		if (itemId == 57)
			count = (int) (count * Config.RATE_QUEST_REWARD_ADENA);
		else
			count = (int) (count * Config.RATE_QUEST_REWARD);
	...
	/**
	 * Drop Quest item using Config.RATE_QUEST_DROP and random number.<br>
	...
	/**
	 * Drop Quest item using Config.RATE_QUEST_DROP and random number.
	 * @param itemId : int Item Identifier of the item to be dropped
	 ...
	 * @param npcId template of the npc.
	...
	/**
	 * Add spawn for player instance Will despawn after the spawn length expires Uses player's coords and heading. Adds a little randomization in the x y coords
	 * @param npcId
	 * @param cha
	...
	{
		String text = HtmCache.getInstance().getHtm("data/scripts/quests/Tutorial/" + html);
		if (text == null)
		{
			_log.warning("missing html page data/scripts/quests/Tutorial/" + html);
			text = "<html><body>File data/scripts/quests/Tutorial/" + html + " not found or file is empty.</body></html>";
		}
