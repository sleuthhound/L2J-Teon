			...
			if (target.isSeeded())
			{
				if (calcSuccess(player, target))
				{
					...
					if (items != null && items.length > 0)
					{
						...
						if (send)
						{
							player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_PICKED_UP_S1_S2).addNumber(total).addItemName(cropId));
							
							if (player.isInParty())
								player.getParty().broadcastToPartyMembers(player, SystemMessage.getSystemMessage(SystemMessageId.S1_HARVESTED_S3_S2S).addPcName(player).addNumber(total).addItemName(cropId));
