...
public class CursedWeaponsManager
{
	...
	try
	{
		...
		try
		{
			 ...
			 * character_skills WHERE char_obj_id=? AND skill_id=");
	...
	if (player.isCursedWeaponEquiped()) // cannot own 2 cursed swords
	{
	     ...
	     * following code adds "_stageKills" levels. This will also show
	     * in the char status. I do not have enough info to know if the
	     * bonus should be shown in the pk count, or if it should be a
	     * full "_stageKills" bonus or just the remaining from the
	     * current count till the of the current stage... This code is a
	     * TEMP fix, so that the cursed weapon's bonus level can be
	     * observed with as little change in the code as possible, until
	     * proper info arises.
	     */
