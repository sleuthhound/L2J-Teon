...
public class SummonFriend implements ISkillHandler
{
	...
	try
	{
		...
		if (target instanceof L2PcInstance)
		{
		    ...
		    if (targetChar.isNoob())
		    {
			activePlayer.sendMessage("You may not use an Summon Friend skill in a Faction mode.");
			activePlayer.sendPacket(new ActionFailed());
			return;
		    }
		    if (targetChar.isKoof())
		    {
			activePlayer.sendMessage("You may not use an Summon Friend skill in a Faction mode.");
			activePlayer.sendPacket(new ActionFailed());
			return;
		    }
		    // Target cannot be in combat (or dead, but that's
		    // checked
		    // by TARGET_PARTY)
		    ...
		    if ((targetChar.getInventory().getItemByItemId(8615) == null) && (skill.getId() != 1429) && (skill.getId() != 3632))
		    {
		    ...
		    if (!Util.checkIfInRange(0, activeChar, target, false))
		    {
		    	switch (skill.getId()) {
	            	case 1429:
	            		targetChar.sendPacket(SystemMessage.sendString("Byl jsi privolan k parte."));
				        targetChar.teleToLocation(activeChar.getX(), activeChar.getY(), activeChar.getZ(), true);
				        break;
				    case 3632:
				        targetChar.sendPacket(SystemMessage.sendString("Byl jsi privolan ke krali."));
				        targetChar.teleToLocation(activeChar.getX(), activeChar.getY(), activeChar.getZ(), true);
				        break;
				    default:
