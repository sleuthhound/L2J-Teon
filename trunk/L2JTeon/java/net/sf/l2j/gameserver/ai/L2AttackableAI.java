		...
		if (_globalAggro >= 0)
		{
			...
			// synchronized (npc.getKnownList().getKnownObjects())
			{
				for (L2Object obj : objs)
				{
					...
					/*
					 * Temporarily adding this commented code as a concept to be used eventually. However, the way it is written below will NOT work correctly. The NPC should only notify Aggro Range Enter when someone enters the range from outside. Instead, the below code will keep notifying even while someone remains within the range. Perhaps we need a short knownlist of range = aggroRange for
					 * just people who are actively within the npc's aggro range?...(Fulminus) // notify AI that a playable instance came within aggro range if ((obj instanceof L2PcInstance) || (obj instanceof L2Summon)) { if ( !((L2Character)obj).isAlikeDead() && !npc.isInsideRadius(obj, npc.getAggroRange(), true, false) ) { L2PcInstance targetPlayer = (obj instanceof L2PcInstance)?
					 * (L2PcInstance) obj: ((L2Summon) obj).getOwner(); if (npc.getTemplate().getEventQuests(Quest.QuestEventType.ON_AGGRO_RANGE_ENTER) !=null) for (Quest quest: npc.getTemplate().getEventQuests(Quest.QuestEventType.ON_AGGRO_RANGE_ENTER)) quest.notifyAggroRangeEnter(npc, targetPlayer, (obj instanceof L2Summon)); } }
					 */
