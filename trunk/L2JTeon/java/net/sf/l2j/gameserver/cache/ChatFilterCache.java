...
public class ChatFilterCache
{
    ...
    /*
     * GameServer not use this functions now... will be added soon...
     * papient... DO NOT DELETE THIS FUNCTIONS FROM THE SOURCE - WORK IS IN
     * PROGRESS!!! public static void addWord(String textData) {
     * _instance.addWordInDB(textData); } private void addWordIntoDB(String
     * textData) { java.sql.Connection con = null; try { con =
     * L2DatabaseFactory.getInstance().getConnection(); PreparedStatement
     * statement = con.prepareStatement("INSERT INTO chat_filter
     * (filterWord) VALUES (?)"); statement.setString(1, textData);
     * statement.executeUpdate(); statement.close(); } catch (SQLException
     * e) { _log.warning("Could not add new filter word into DB: " + e); }
