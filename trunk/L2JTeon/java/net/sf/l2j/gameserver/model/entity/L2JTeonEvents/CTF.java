...
public class CTF
{
	public static String _eventName = new String("Capture The Flag"), _eventDesc = new String("You must capture the opposing Flag and bring it back to your Flag to score."), _topTeam = new String(), _joiningLocationName = new String("Giran");
	...
	public static Vector<Integer> _teamPlayersCount = new Vector<Integer>(), _teamPointsCount = new Vector<Integer>(), _teamColors = new Vector<Integer>(), _flagIds = new Vector<Integer>(), _flagsX = new Vector<Integer>(), _flagsY = new Vector<Integer>(), _flagsZ = new Vector<Integer>();
	public static Vector<L2Spawn> _flagSpawns = new Vector<L2Spawn>();
	public static Vector<Boolean> _flagsTaken = new Vector<Boolean>();
	public static boolean _joining = false, _teleport = false, _started = false, _finished = false, _sitForced = false;
	public static L2Spawn _npcSpawn;
	public static int _npcId = 70012, _npcX = 0, _npcY = 0, _npcZ = 0, _npcHeading = 180, _rewardId = 57, _rewardAmount = 1000, _topScore = 0;
	...
	/**
	 * This initiates the participation period which allows players to join the event.
	 */
	...
	/**
	 * This teleports all players participating in the event to their respective starting locations.
	 */
		...
		private void processInFlagRange()
		{
			for (String team : CTF._teams)
			{
				if (team.equals(_player._teamNameCTF))
				{
					...
					if ((_player.getX() > CTF._flagsX.get(indexOwn) - 100) && (_player.getX() < CTF._flagsX.get(indexOwn) + 100) && (_player.getY() > CTF._flagsY.get(indexOwn) - 100) && (_player.getY() < CTF._flagsY.get(indexOwn) + 100) && (_player.getZ() > CTF._flagsZ.get(indexOwn) - 100) && (_player.getZ() < CTF._flagsZ.get(indexOwn) + 100) && !CTF._flagsTaken.get(indexOwn)
							&& _player._haveFlagCTF)
					{
				...
				else
				{
					...
					if ((_player.getX() > CTF._flagsX.get(indexEnemy) - 100) && (_player.getX() < CTF._flagsX.get(indexEnemy) + 100) && (_player.getY() > CTF._flagsY.get(indexEnemy) - 100) && (_player.getY() < CTF._flagsY.get(indexEnemy) + 100) && (_player.getZ() > CTF._flagsZ.get(indexEnemy) - 100) && (_player.getZ() < CTF._flagsZ.get(indexEnemy) + 100) && !CTF._flagsTaken.get(indexEnemy)
							&& !_player._haveFlagCTF && !_player.isDead())
					{
