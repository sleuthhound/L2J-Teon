...
public class L2RaidEvent
{
     ...
     */
    private static int _first_id, _first_ammount, _second_id, _second_ammount,
	    _event_ammount;
    ...
    /**
     * Vector Created to add Single/Party/Clan Players onto the event. TODO:
     * Use this vector also to add another kind of event --> free Event with
     * any player that may want to participate.
     */
    public static Vector<L2PcInstance> _participatingPlayers = new Vector<L2PcInstance>();
    /**
     * Players from whom we're waiting for an answer in order to know it
     * they want to join the event.
     */
     ...
     * @param type
     *                --> Type of Event: 1: Single Event || 2: Clan Event ||
     *                3: Party Event
     * @param points
     ...
     *                --> Amount of Mobs
     * @param minPeople
     *                --> Minimum People required to run event (Only
     *                functional on Clan and Party Events)
     * @param bufflist
     *                --> BuffList to apply to the player. Defined in the
     *                SQL table buff_templates
     * @param rewardLevel
     *                --> Reward level to apply upon player's victory.
     * @param effector
     *                --> Effector of the Buffs (Previously defined in
     *                L2EventMAnagerInstance.java)
     ...
     * <li> We don't need to check if the player can or can not have access
     * to the buffing state since it has previously been checked.
     * <li> We assign a value of previousEventPoints to notify the player.
     * <li> Apply the buffs.
     * <li> Notify the player once he/she has gotten the Buffs. <br>
     * More Documentation can Be found inside the method's code.<br>
     * We will apply the buffs previous to the Event following the
     * parameters:
     * 
     * @param player
     *                --> Player participating in the Event.
     * @param eventPoints
     *                --> Event points to be deduced once the buffing takes
     *                place.
     * @param buffList
    ...
    {
	/*
	 * Check if the event type has been defined. Once the event is fully
	 * functional this checks will be taken out
	 */
	if ((_eventType != 1) && (_eventType != 2) && (_eventType != 3))
	{
	...
	if (_eventType == 2)
	{
		 ...
		 * subject has enough Event Points as to pay the Buffs, and
		 * enroll into the event. 2. If that's not the case we will
		 * proceed into the first IF: 2a. The even points will be
		 * replaced by 0 since the player doesn't have enough event
		 * points to pay the normal quota. 2b. We will notify him of
		 * this situation. We will also deduce the missing points from
		 * other Clan Members. (Sharing is good right? xD) 3. If 1 is
		 * Affirmative we will proceed onto the second IF: 3a. Deduction
		 * of event points = to what's needed to participate in the
		 * event/online clan members. 3b. Notify this situation and
		 * inform the player of the amount of points that he/she has at
		 * this very moment.
		 */
     ...
     * This method checks it the total amount of events in process is > than
     * the allowed and acts according to that and the parameters given.<br>
     * In case X events are already taking place, the void returns and won't
     * let us continue with the event.<br>
     * This check is not needed since we already check this in
     * L2EventManagerInstance.java, but i'll leave it in here for now since
     * this is a very early stage of developing for now.<br>
    ...
    /**
     * Teleport the event participants to where the event is going to take
     * place<br>
     * A function has been created to make it easier for us to teleport the
     * players every time we need them to teleport.<br>
     ...
     * Added Support for multiple spawns and for each one of them being
     * defined as Event Mob.
     * 
     * @param monsterId
     *                --> Npc Id
     * @param respawnTime
     *                --> Respawn Delay (in most cases this will be 0 as
     *                we're gonna cut the respawning of the Mobs upon
     *                death).
     * @param mobCount
	...
	if (mobCount > 1)
	{
		...
		try
		{
		    ...
		    spawn.init();
		    /*
		     * Define the properties of every spawn. TODO: Change
		     * the Mob statistics according on Event Participants
		     * and Server Rates.
		     */
    ...
    /**
     * Function launched at every player death (if he/she's enrolled in any
     * Raid event)
     * 
     * @param player
     */
    ...
    {
	/*
	 * TODO: Add support for: - Configurable Death rebirth system including: -
	 * Automatic respawn (Done) - Track deaths for player. - doRevive?
	 * (Done)
	 */
    ...
    /**
     * This is the place where we define all the actions that take place
     * after one Event Mob dies. a. Check if that was the last event mob of
     * this instance. b. If not, decrease the number by one. c. Else return
     * true.
     */
    ...
    /**
     * This void picks the rewards and launches the hand out system. It also
     * Ends the event. Added database support for this.
     * 
     * @param player
     *                --> Player taking the action.
     */
    ...
    /**
     * Custom Definition of the Experience. TODO: Add custom definitions of
     * Experience for different prize lists.
     */
    ...
    /**
     * Load Data of the prizes for each event. Added DataBase support for
     * this.
     * 
     * @param prizePackage
     */
