...
public class L2Multisell
{
    ...
    /**
     * This will generate the multisell list for the items. There exist
     * various parameters in multisells that affect the way they will
     * appear: 1) inventory only: * if true, only show items of the
     * multisell for which the "primary" ingredients are already in the
     * player's inventory. By "primary" ingredients we mean weapon and
     * armor. * if false, show the entire list. 2) maintain enchantment:
     * presumably, only lists with "inventory only" set to true should
     * sometimes have this as true. This makes no sense otherwise... * If
     * true, then the product will match the enchantment level of the
     * ingredient. if the player has multiple items that match the
     * ingredient list but the enchantment levels differ, then the entries
     * need to be duplicated to show the products and ingredients for each
     * enchantment level. For example: If the player has a crystal staff +1
     * and a crystal staff +3 and goes to exchange it at the mammon, the
     * list should have all exchange possibilities for the +1 staff,
     * followed by all possibilities for the +3 staff. * If false, then any
     * level ingredient will be considered equal and product will always be
     * at +0 3) apply taxes: Uses the "taxIngredient" entry in order to add
     * a certain amount of adena to the ingredients
     * 
