...
public class Valakas extends L2AttackableAIScript
{
	...
	public Valakas(int questId, String name, String descr)
	{
		...
		else
		{
			...
			else
			{
				...
				if (status == WAITING)
					startQuestTimer("beginning", Config.WAIT_TIME_VALAKAS, valakas, null);
			}
		}
	}
	...
	private void callSkillAI(L2Npc npc)
	{
		...
		// Pickup a target if no, dead or offline victim.
		if (_actualVictim == null || _actualVictim.isDead() || !(_Zone.isCharacterInZone(_actualVictim)))
	...
	private int getRandomSkill(L2Npc npc)
	{
		...
		if (Util.getPlayersCountInRadius(600, npc, true, false) >= 20)
	...
	private L2Playable getRandomTarget()
	{
		...
		for (L2Playable obj : _Zone.getPlayableInside(false))
		{
			if (!obj.isDead())
				result.add(obj);
		}
