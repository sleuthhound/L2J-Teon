...
public class ItemTable
{
     ...
     * 
     * @param rset :
     *                ResultSet designating a record of the [weapon] table
     *                of database
     ...
     * 
     * @param rset :
     *                ResultSet designating a record of the [etcitem] table
     *                of database
     * @return Item : object created from the database record
     * @throws SQLException
     */
     ...
     * _etcItems.put(item.getItemId(), item); } } private void
     * fillArmorsTable() { List<L2Armor> armorList =
     * SkillsEngine.getInstance().loadArmors(armorData); /*for (Item
     * itemInfo : armorData.values()) { L2Armor armor =
     * SkillsEngine.getInstance().loadArmor(itemInfo.id, itemInfo.type,
     * itemInfo.name, itemInfo.set); if (armor == null) armor = new
     * L2Armor((L2ArmorType)itemInfo.type, itemInfo.set);
     * _armors.put(armor.getItemId(), armor); }* } private void
     * FillWeaponsTable() { for (Item itemInfo : weaponData.values()) {
     * L2Weapon weapon = SkillsEngine.getInstance().loadWeapon(itemInfo.id,
     * itemInfo.type, itemInfo.name, itemInfo.set); if (weapon == null)
     * weapon = new L2Weapon((L2WeaponType)itemInfo.type, itemInfo.set);
     * _weapons.put(weapon.getItemId(), weapon); } }
     */
    /**
     * Builds a variable in which all items are putting in in function of
     * their ID.
     */
     ...
     * @param count :
     *                int Quantity of items to be created for stackable
     *                items
     * @param actor :
