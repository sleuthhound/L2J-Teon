...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst, L2Character target)
	{
		...
		// The skill can only be used on the L2Character targeted, or on the caster itself
			 ...
			 * // npc only for now - untested case TARGET_CLAN_MEMBER: { if (activeChar instanceof L2Npc) { // for buff purposes, returns friendly mobs nearby and mob itself final L2Npc npc = (L2Npc) activeChar; if (npc.getClan() == null || npc.getClan().isEmpty()) { return new
			 * L2Character[]{activeChar}; } final Collection<L2Object> objs = activeChar.getKnownList().getKnownObjects().values(); for (L2Object newTarget : objs) { if (newTarget instanceof L2Npc && npc.getClan().equals(((L2Npc) newTarget).getClan())) { if (!Util.checkIfInRange(getCastRange(),
			 * activeChar, newTarget, true)) continue; if (((L2Npc) newTarget).getFirstEffect(this) != null) continue; targetList.add((L2Npc) newTarget); break; // found } } if (targetList.isEmpty()) targetList.add(npc); } else return _emptyTargetList; return targetList.toArray(new
			 * L2Character[targetList.size()]); }
			 */
			default:
			{
